package com.mark.sdklib.imageLoader;
 ...
 * 初始化 UniverImageLoader,并用来加载网络图片
 */

public class ImageLoaderManager {

    private static final int THREAD_COUNT = 4;//UIL最多可以有多少条线程
    ...
    private DisplayImageOptions getDefultOptions() {
        ...
        DisplayImageOptions options = new DisplayImageOptions.Builder()
                .showImageForEmptyUri(android.R.mipmap.sym_def_app_icon)//在图片地址为空的时候
                .showImageOnFail(android.R.mipmap.sym_def_app_icon)//图片下载失败的时候
                .cacheInMemory(true)//设置图片可以缓存在内存
                .cacheOnDisk(true)//设置图片可以缓存在硬盘
                .bitmapConfig(Bitmap.Config.RGB_565)//使用的图片解码类型
                .decodingOptions(new BitmapFactory.Options())//是图片解码的配置
                .build();
