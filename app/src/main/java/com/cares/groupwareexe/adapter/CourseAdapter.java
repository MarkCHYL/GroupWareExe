package com.cares.groupwareexe.adapter;
...
import com.cares.groupwareexe.utils.Utils;
import com.mark.sdklib.imageLoader.ImageLoaderManager;
...
public class CourseAdapter extends BaseAdapter {
    ...
    private ArrayList<BaseRecommandModel.DataBean.ListBean> mData;
    private ViewHolder mViewHolder;
//    private VideoAdContext mAdsdkContext;
    private ImageLoaderManager mImagerLoader;

    public CourseAdapter(Context context, ArrayList<BaseRecommandModel.DataBean.ListBean> data) {
        mContext = context;
        mData = data;
    ...
    public int getItemViewType(int position) {
        BaseRecommandModel.DataBean.ListBean value = (BaseRecommandModel.DataBean.ListBean) getItem(position);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final BaseRecommandModel.DataBean.ListBean value = (BaseRecommandModel.DataBean.ListBean) getItem(position);
        //无tag时
        if (convertView == null) {
            switch (type) {
                case VIDOE_TYPE:
...
//                    });
                ...
                case CARD_TYPE_THREE:
                    ...
                    //add data
                    ArrayList<BaseRecommandModel.DataBean.ListBean> recommandList = Utils.handleData(value);
        ...
        switch (type) {
            case VIDOE_TYPE:
                mImagerLoader.displayImage(mViewHolder.mLogoView, value.getLogo());
                mViewHolder.mTitleView.setText(value.getTitle());
                mViewHolder.mInfoView.setText(value.getInfo().concat(mContext.getString(R.string.tian_qian)));
                mViewHolder.mFooterView.setText(value.getText());
...
//                        dialog.show();
            ...
            case CARD_TYPE_ONE:
                ...
                mViewHolder.mInfoView.setText(value.getInfo().concat(mContext.getString(R.string.tian_qian)));
                mViewHolder.mFooterView.setText(value.getText());
                mViewHolder.mPriceView.setText(value.getPrice());
                mViewHolder.mFromView.setText(value.getFrom());
                mViewHolder.mZanView.setText(mContext.getString(R.string.dian_zan).concat(value.getZan()));
...
//                        mContext.startActivity(intent);
            ...
            case CARD_TYPE_TWO:
                ...
                mViewHolder.mZanView.setText(mContext.getString(R.string.dian_zan).concat(value.getZan()));
                //为单个ImageView加载远程图片
                mImagerLoader.displayImage(mViewHolder.mProductView, value.getUrl().get(0));
                break;
            case CARD_TYPE_THREE:
                break;
        }
